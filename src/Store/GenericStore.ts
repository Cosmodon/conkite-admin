...
class APIClass {
	async get(apiName: string, path: string, body: MessageBody): Promise<ResponseObject> {
		let result: ResponseObject = {
			data: []
		};
		return result;
	}
	async put(apiName: string, path: string, body: MessageBody): Promise<ResponseObject> {
		let result: ResponseObject = {
			data: []
		};
		return result;
	}
	async post(apiName: string, path: string, body: MessageBody): Promise<ResponseObject> {
		let result: ResponseObject = {
			data: []
		};
		return result;
...
class GenericStore {
	...
	processErrorCode = (error) => {
		...
		const errorText = error.response.data.message || error.response.data.statusText || error.response.data.error || 'unknown';
		const errorCode = error.response.data.code || error.response.data.statusCode;

		if ((errorCode >= 400 && errorCode < 500) || (errorCode >= 400 && errorCode < 500)) {
	...
	check = async () => {
		...
		return API.get(this.apiName, path, message).then(response => {
	...
	update = async (item) => {
		...
		return API.put(this.apiName, path, message).then(response => {
	...
	create = async (item) => {
		...
		return API.post(this.apiName, path, message).then(response => {
